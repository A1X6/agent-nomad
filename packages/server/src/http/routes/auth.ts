...
/** POST /auth/prelogin, /auth/register, /auth/login, /auth/logout (T15, limits T18). */
export function authRoutes(
  auth: AuthService,
  { limiter, clientIp }: AuthRouteLimits,
): Hono<{ Variables: SessionVariables }> {
  const perIp = (rule: RateLimitRule) => limitPerIp(limiter, rule, clientIp);
  return new Hono<{ Variables: SessionVariables }>()

    .post(
      API_ROUTES.prelogin,
      smallBody(),
      perIp(RATE_LIMITS.authPerIp),
      jsonBody(PreloginRequestSchema),
      async (c) => {
    ...
    .post(
      ...
      smallBody(),
      perIp(RATE_LIMITS.authPerIp),
      perIp(RATE_LIMITS.registerPerIp),
      jsonBody(RegisterRequestSchema),
      async (c) => {
